...
class InvokePlugin implements PluginInterface
{
    public function handle(Patchwerk $patchwerk, \Closure $next): Patchwerk
    {
        ...
        if (in_array($destination['body']['state'], TransferStateEnum::isFail())) {
            throw new InvalidResponseException(TransferStateEnum::{$destination['body']['state']}->name());
        }

        $config = Wechat::getConfig();

        $response = [
            'response' => $destination['body'],
            'jsapi' => [
                'mchId' => $config['mch_id'],
